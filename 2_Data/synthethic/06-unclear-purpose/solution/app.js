...
document.querySelectorAll('.action-link').forEach(control=>control.addEventListener('click',event=>{event.preventDefault();toast.textContent=control.textContent.trim()||'Opened';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1200)}));
